defmodule EnergyTreeWeb.EnergyTreeLiveView.State do
  ...
  def sign_in(state, user_id, preferences) do
    new_navigation =
      state.navigation
      |> Navigation.navigate_to(if preferences.onboarded?, do: :dashboard, else: :onboarding1)
  ...
  def simulate_update_battery_level(state = %{traffic_light: :red}) do
    if state.preferences && state.preferences.minimum_battery < state.battery_level do
